...
const subscriptionCheck = async (req, res, next) => {
  try {
    ...
    const moduleAccess = await Module.findOne({ tenantId: tenant._id, moduleName: 'apartment', status: 'active' });
    if (!moduleAccess) return next(new ApiError(403, 'Apartment module not active. Please subscribe.', 'MODULE_INACTIVE'));
