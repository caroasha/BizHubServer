...
const subscriptionCheck = async (req, res, next) => {
  try {
    ...
    const moduleAccess = await Module.findOne({ tenantId: tenant._id, moduleName: 'electro', status: 'active' });
    if (!moduleAccess) return next(new ApiError(403, 'Electronics module not active. Please subscribe.', 'MODULE_INACTIVE'));
