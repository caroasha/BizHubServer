...
const subscriptionCheck = async (req, res, next) => {
  try {
    ...
    const moduleAccess = await Module.findOne({ tenantId: tenant._id, moduleName: 'pharma', status: 'active' });
    if (!moduleAccess) return next(new ApiError(403, 'Pharmacy module not active. Please subscribe.', 'MODULE_INACTIVE'));
