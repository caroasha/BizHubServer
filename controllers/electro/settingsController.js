const ElectroSettings = require('../../models/electro/Settings');
...
const getSettings = asyncHandler(async (req, res) => {
  let settings = await ElectroSettings.findOne({ tenantId: req.tenant._id }).lean();
  if (!settings) settings = await ElectroSettings.create({ tenantId: req.tenant._id });
  const tenant = await Tenant.findById(req.tenant._id).select('businessName contact owner').lean();

  sendSuccess(res, {
    general: {
      shopName: settings.general?.shopName || tenant?.businessName || '',
...
const updateGeneral = asyncHandler(async (req, res) => {
  let settings = await ElectroSettings.findOne({ tenantId: req.tenant._id }) || new ElectroSettings({ tenantId: req.tenant._id });
